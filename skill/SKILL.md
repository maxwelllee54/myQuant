...
本技能V2.0不仅融入了前沿的量化理论，更整合了六位传奇投资大师的智慧，将经典的投资哲学与现代量化方法相结合，构建了一个兼具理论深度和实战价值的投资框架。

### 2. 六位大师概览

| 大师 | 核心贡献 | 可量化标准 |
|------|---------|-----------|
| **本杰明·格雷厄姆** | 价值投资之父，安全边际理论 | P/E<15, P/B<1.5, 安全边际>33% |
| **沃伦·巴菲特** | 护城河理论，长期持有 | ROE>15%(5年), 低债务, 强FCF |
| **彼得·林奇** | PEG策略，成长股投资 | PEG<1, 盈利增长>20% |
| **Ray Dalio** | 风险平价，全天候策略 | 相关性<0.2, 风险贡献平衡 |
| **Michael Burry** | 逆向投资，深度研究 | 市场悲观+强基本面 |
| **霍华德·马克斯** | 第二层思维，周期判断 | 市场周期定位，风险vs回报 |

### 3. 实现方式

#### 3.1 多维度评分系统

**`scripts/master_strategies.py`** 实现了一个综合评分系统，将六位大师的标准整合为统一的评分框架：
...
# - scores: 各大师维度的得分
# - cycle_phase: 当前市场周期阶段
```

#### 3.2 动态权重调整

根据霍华德·马克斯的市场周期理论，系统会自动调整各大师策略的权重：
...
1. **格雷厄姆筛选**：过滤明显高估的股票，建立安全边际候选池
2. **巴菲特质量检验**：评估护城河和竞争优势
3. **林奇成长评估**：计算PEG比率，识别成长潜力
4. **Burry逆向机会**：检查市场情绪，寻找被错杀的机会
5. **马克斯周期定位**：评估当前市场位置，调整仓位大小
6. **Dalio组合构建**：计算相关性，优化风险平衡
...
```bash
python3 /home/ubuntu/skills/quant-investor/scripts/master_strategies.py \
    --symbol AAPL \
...
# 加载股票池数据
stocks_df = pd.read_csv(\'stock_pool.csv\')

# 批量评分
strategies = MasterStrategies()
results = strategies.batch_screen(stocks_df, market_data, sentiment_data)

# 结果按总分排序，可直接用于选股
top_picks = results[results[\'recommendation\'].isin([\'STRONG_BUY\', \'BUY\'])]
```

### 5. 参考文档

详细的投资大师理念、量化标准和实战策略，请参阅：

**`references/master_investors_wisdom.md`**
...
- 综合策略框架和组合构建方法
- 风险管理和卖出时机的大师智慧
- 常见组合策略（保守型、成长型、逆向型、全天候型）

### 6. 核心启示

1. **没有完美的策略**：每位大师都有其局限性和适用场景
2. **组合多种智慧**：取长补短，构建更稳健的投资系统
3. **适应市场周期**：不同环境下侧重不同策略
4. **保持学习**：市场在变化，策略也要进化
5. **纪律和耐心**：知道和做到之间有巨大差距

通过将投资大师的智慧与现代量化方法相结合，本技能为您提供了一个既有理论深度又有实战价值的完整投资框架。


## 附录F：Point-in-Time数据与动态复权

### F.1 问题：前复权的Look-Ahead Bias

传统的前复权价格在计算时，会使用"当前时点"之后的所有分红信息。这导致在历史回测中，我们使用了未来才会发生的分红数据，从而引入了**前视偏差（Look-Ahead Bias）**。
...
为了彻底消除Look-Ahead Bias，本技能V2.2版本引入了**Point-in-Time回测框架**，核心思想是：

1. **使用真实价格（不复权）**：从数据源获取不复权的价格数据。
2. **单独处理分红**：在回测的每一天，检查当天是否有分红事件，并将分红收益直接加到现金账户中。
...
本技能新增了`scripts/point_in_time_data.py`模块，包含：

- `PointInTimeDataManager`：用于获取真实价格和分红数据
- `PointInTimeBacktester`：全新的回测引擎，确保在回测的每一天只使用该日之前的信息

**推荐的回测模式**：

```python
from scripts.point_in_time_data import PointInTimeDataManager, PointInTimeBacktester

# 1. 初始化数据管理器
data_manager = PointInTimeDataManager()

# 2. 初始化回测引擎（使用真实价格模式）
backtester = PointInTimeBacktester(data_manager, use_dynamic_adjustment=False)
...
- **短期指标**（如MA5, MA10）：直接使用真实价格计算
- **长期指标**（如MA200）：使用动态前复权价格计算，以平滑除权缺口
- **相对指标**（如RSI, MACD）：使用真实价格计算

通过这次升级，`quant-investor`技能的回测框架现在完全符合学术界和业界的最高标准，确保了所有量化分析结果的**严谨性、准确性和可复现性**。
